...
def parse_arguments():
    ...
    parser.add_argument('--model_path', type=str, required=True,
                        help='Đường dẫn đến mô hình đã huấn luyện (Q-Learning hoặc SARSA)')
    
    parser.add_argument('--model_type', type=str, choices=['q_learning', 'sarsa'],
    ...
    parser.add_argument('--compare_type', type=str, choices=['q_learning', 'sarsa'],
...
def create_maze_generator(maze_type: str, maze_size: int) -> Any:
    ...
    Args:
        maze_type: Loại mê cung ('dfs', 'prim', 'wilson')
        maze_size: Kích thước mê cung (NxN)
        
    Returns:
        Bộ sinh mê cung tương ứng
    """
    if maze_type == 'dfs':
...
def create_agent(model_type: str, state_size: Tuple[int, int], action_size: int) -> Any:
    ...
    Args:
        model_type: Loại mô hình ('q_learning', 'sarsa')
...
def evaluate_model(agent: Any, env: MazeEnvironment, num_episodes: int, max_steps: int,
                   render: bool = False, render_delay: float = 0.1, noise_level: float = 0.0) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        render: Hiển thị mê cung trong quá trình đánh giá
        render_delay: Độ trễ giữa các bước khi hiển thị
        noise_level: Mức độ nhiễu (xác suất thực hiện hành động ngẫu nhiên)
    ...
    for episode in tqdm(range(num_episodes), desc="Evaluating episodes"):
        ...
        while not done and steps < max_steps:
            ...
            if np.random.random() < noise_level:
                # Thực hiện hành động ngẫu nhiên với xác suất noise_level
                action = np.random.randint(0, env.get_action_size())
            else:
                # Sử dụng chính sách đã học
        ...
        # Tăng tỷ lệ thành công nếu đạt đến đích
        if done and info.get('status') == 'reached_goal':
            success_rate += 1
...
def visualize_results(env: MazeEnvironment, agent: Any, results: Dict[str, Any], output_dir: str) -> None:
    """
    ...
    # 1. Hiển thị chính sách tốt nhất
...
def compare_models(agent1: Any, agent2: Any, env: MazeEnvironment, num_episodes: int, max_steps: int,
    ...
    Args:
        agent1: Agent thứ nhất
        agent2: Agent thứ hai
        env: Môi trường mê cung
        num_episodes: Số episode đánh giá
        max_steps: Số bước tối đa trong mỗi episode
        output_dir: Thư mục lưu kết quả
    ...
    results1 = evaluate_model(agent1, env, num_episodes, max_steps)
    
    print("\nEvaluating second model...")
    # Reset môi trường để đảm bảo công bằng
    env.reset()
    results2 = evaluate_model(agent2, env, num_episodes, max_steps)
    ...
    plt.subplot(2, 2, 1)
    plt.hist(results1['steps'], bins=20, alpha=0.5, label='Model 1')
    plt.hist(results2['steps'], bins=20, alpha=0.5, label='Model 2')
    plt.axvline(results1['avg_steps'], color='blue', linestyle='--', label=f'Model 1 Avg: {results1["avg_steps"]:.2f}')
    plt.axvline(results2['avg_steps'], color='orange', linestyle='--', label=f'Model 2 Avg: {results2["avg_steps"]:.2f}')
    ...
    plt.subplot(2, 2, 2)
    plt.hist(results1['rewards'], bins=20, alpha=0.5, label='Model 1')
    plt.hist(results2['rewards'], bins=20, alpha=0.5, label='Model 2')
    plt.axvline(results1['avg_reward'], color='blue', linestyle='--', label=f'Model 1 Avg: {results1["avg_reward"]:.2f}')
    plt.axvline(results2['avg_reward'], color='orange', linestyle='--', label=f'Model 2 Avg: {results2["avg_reward"]:.2f}')
    ...
    plt.title('Value Function Difference (Model 2 - Model 1)')
    plt.colorbar(im)
    ...
    plt.colorbar(ticks=[0, 1], labels=['Same', 'Different'])
    ...
    # Hiển thị thông tin tổng hợp
    print("\n===== MODEL COMPARISON =====")
    print(f"Model 1 Success Rate: {results1['success_rate']:.2f}%")
    print(f"Model 2 Success Rate: {results2['success_rate']:.2f}%")
    print(f"Model 1 Average Reward: {results1['avg_reward']:.2f}")
    print(f"Model 2 Average Reward: {results2['avg_reward']:.2f}")
    print(f"Model 1 Average Steps: {results1['avg_steps']:.2f}")
    print(f"Model 2 Average Steps: {results2['avg_steps']:.2f}")
...
def test_robustness(agent: Any, maze_type: str, maze_sizes: List[int], num_episodes: int, 
                    max_steps: int, output_dir: str) -> Dict[str, List[float]]:
    """
    Kiểm tra khả năng tổng quát hóa của mô hình trên các mê cung kích thước khác nhau
    
    Args:
        agent: Agent học tăng cường
        maze_type: Loại mê cung
        maze_sizes: Danh sách kích thước mê cung cần kiểm tra
        num_episodes: Số episode đánh giá cho mỗi kích thước
        max_steps: Số bước tối đa trong mỗi episode
        output_dir: Thư mục lưu kết quả
        
    Returns:
        Dictionary chứa kết quả đánh giá trên các kích thước
    """
    success_rates = []
    avg_rewards = []
    avg_steps = []
    ...
    for size in maze_sizes:
        ...
        results = evaluate_model(agent, env, num_episodes // len(maze_sizes), max_steps)
    ...
    plt.subplot(1, 3, 1)
    plt.plot(maze_sizes, success_rates, 'o-', color='blue')
    ...
    plt.subplot(1, 3, 2)
    plt.plot(maze_sizes, avg_rewards, 'o-', color='green')
    ...
    plt.subplot(1, 3, 3)
    plt.plot(maze_sizes, avg_steps, 'o-', color='red')
...
def analyze_path_efficiency(env: MazeEnvironment, agent: Any, num_episodes: int, max_steps: int,
    ...
    Args:
        ...
        output_dir: Thư mục lưu kết quả
    """
    # Tính đường đi ngắn nhất bằng BFS
    optimal_path = env.get_shortest_path()
    optimal_length = len(optimal_path) - 1  # Trừ điểm bắt đầu
    
    # Đánh giá đường đi của agent
    results = evaluate_model(agent, env, num_episodes, max_steps)
    
    # Tính tỷ lệ hiệu quả (optimal_length / agent_steps)
    efficiency_ratios = [optimal_length / steps if steps > 0 else 0 for steps in results['steps']]
...
def main():
    ...
    # Hiển thị kết quả
    visualize_results(env, agent, results, args.output_dir)
    
    # Phân tích hiệu quả đường đi
    analyze_path_efficiency(env, agent, args.num_episodes, args.max_steps, args.output_dir)
    ...
    if args.compare and args.compare_path:
        ...
        # So sánh hai mô hình
        compare_models(agent, compare_agent, env, args.num_episodes, args.max_steps, args.output_dir)
    
    # Kiểm tra khả năng tổng quát hóa
    maze_sizes = [5, 10, 15, 20]  # Danh sách kích thước mê cung cần kiểm tra
    test_robustness(agent, args.maze_type, maze_sizes, args.num_episodes, args.max_steps, args.output_dir)
